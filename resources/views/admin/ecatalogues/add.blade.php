...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <form id="ecatalogueForm" action="{{ $action }}" method="post" class="needs-validation outer-repeater"
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>Picture <span class="required">* The image size must not exceed 2
                                                MB.</span></label>
                                        <div class="custom-file">
                                            <input name="ecatalogue_image" type="file" class="custom-file-input"
                                                id="customFile" accept="image/*" onchange="imgChange(this)"
                                                value="{{ @$ecatalogue->ecatalogue_image }}"
                                                @empty($ecatalogue->ecatalogue_image) required @endempty >
                                            <label class="custom-file-label"
                                                for="customFile">@isset($ecatalogue->ecatalogue_image)
                                            {{ $ecatalogue->ecatalogue_image }} @else Choose Picture @endisset</label>
                                    </div>
                                    <span class="required"> &nbsp; Suitable scale 650 x 884 pixels.</span>
                                </div>
                                <div class="form-group">
                                    <img class="img-thumbnail imagePreview" @if (!isset($ecatalogue->ecatalogue_image)) style="display: none;" @endif alt="200x200"
                                        width="200"
                                        src="{{ URL::asset('upload/ecatalogue/' . @$ecatalogue->ecatalogue_image) }}"
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="custom-file">
                                        <input type="file" name="ecatalogue_file" class="form-control" accept=".pdf"
                                            @empty(@$ecatalogue->ecatalogue_pdf_name) required @endempty ><br>
                                        <a href="/upload/ecatalogue/pdf/{{ @$ecatalogue->ecatalogue_pdf_name }}"
                                            target="_blank"><mark
                                                class="mt-4">{{ @$ecatalogue->ecatalogue_pdf_name }}</mark></a>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="ecatalogue_title" class="form-control" placeholder="Title" value="{{ @$ecatalogue->ecatalogue_title }}" required>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    document.getElementById("ecatalogueForm").addEventListener("submit", function(event) {
...
<script>
    ...
    $(function() {
        uploader = new Dropzone(".dropzone", {
            url: "{{ url('admin/ecatalogue/uploadGallery') }}",
            paramName: "file",
            params: function params(files, xhr, chunk) {
                return {
                    '_token': "{{ csrf_token() }}",
                    'name': $('#ecatalogueName').val()
                };
            },
        ...
        // $.get("{{ url('admin/ecatalogue/gallery') }}/{{ @$ecatalogue->id }}", function(data) {
