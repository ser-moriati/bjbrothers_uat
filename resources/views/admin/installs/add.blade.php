...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <form id="installForm" action="{{$action}}" method="post" class="needs-validation outer-repeater" novalidate="" enctype="multipart/form-data">
    ...
    <div class="col-sm-12">
        <div class="form-group">
                                ...
                                <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select required name="category_id" id="category" class="form-control">
                                                ...
                                                @foreach ($install_category as $item)
                                                    <option @if (isset($install->ref_category_id)) @if ($install->ref_category_id == $item->id) selected @endif @endif value="{{$item->id}}">{{$item->install_category_name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    <div class="form-group">
                                        <label for="installName">Name <span class="required">*</span></label>
                                        <input type="text" name="install_name" class="form-control" id="installName" placeholder="name" value="{{@$install->install_name}}" required>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="elm1">Detail</label>
                                        <textarea name="install_detail" placeholder="Detail" id="elm1">{{@$install->install_detail}}</textarea>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="custom-file">
                                                <input name="install_image" type="file" class="custom-file-input" id="customFile" accept="image/*" onchange="imgChange(this)" value="{{@$install->install_image}}" @empty($install->install_image) required @endempty >
                                                <label class="custom-file-label" for="customFile">@isset($install->install_image) {{$install->install_image}} @else Choose Picture @endisset</label>
                                            </div>
                                        <span class="required"> &nbsp; Suitable scale 800 x 533 pixels.</span>
                                        </div>
                                        <div class="form-group">
                                            <label for="nameImg">Name image <span class="required">*</span></label>
                                            <input type="text" class="form-control" value="{{pathinfo(@$install->install_image, PATHINFO_FILENAME)}}" id="nameImg" name="install_image_name" required>
                                        </div>
                                        {{-- <div class="col-md-12"> --}}
                                            <div class="form-group">
                                                <img class="img-thumbnail imagePreview"@if(!isset($install->install_image)) style="display: none;" @endif alt="200x200" width="200" src="{{ URL::asset('upload/install/'.@$install->install_image) }}" data-holder-rendered="true">
                                            </div>
                                        {{-- </div> --}}
                                    </div>
                                {{-- </div> --}}
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input id="metatitle" name="meta_title" type="text" placeholder="Detailed Title" class="form-control" value="{{@$install->meta_title}}">
                                </div>
                                <div class="form-group">
                                    <label for="metakeywords">Meta Keywords</label>
                                    <input id="metakeywords" name="meta_keywords" type="text" placeholder="Detailed Keywords" class="form-control" value="{{@$install->meta_keywords}}">
                                </div>
                            </div>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" id="meta_description" name="meta_description" placeholder="Meta Description" rows="5">{{@$install->meta_description}}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {{-- <div class="row"@if (!isset($install))
                style="
                pointer-events: none;
                opacity: 0.4;"
            @endif>
        ...
        <script>
            ...
            $(function(){
                uploader = new Dropzone(".dropzone",{
                    url: "{{url('admin/install/uploadGallery')}}",
                    paramName : "file",
                    params: function params(files, xhr, chunk) { return { '_token' : "{{csrf_token()}}",'name':$('#installName').val() }; },
                ...
                // $.get("{{url('admin/install/gallery')}}/{{@$install->id}}", function(data) {
