...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <form id="maintenanceForm" action="{{$action}}" method="post" class="needs-validation outer-repeater" novalidate="" enctype="multipart/form-data">
    ...
    <div class="col-sm-12">
        <div class="form-group">
                                ...
                                <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select required name="category_id" id="category" class="form-control">
                                                ...
                                                @foreach ($maintenance_category as $item)
                                                    <option @if (isset($maintenance->ref_category_id)) @if ($maintenance->ref_category_id == $item->id) selected @endif @endif value="{{$item->id}}">{{$item->maintenance_category_name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    <div class="form-group">
                                        <label for="maintenanceName">Name <span class="required">*</span></label>
                                        <input type="text" name="maintenance_name" class="form-control" id="maintenanceName" placeholder="name" value="{{@$maintenance->maintenance_name}}" required>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="elm1">Detail</label>
                                        <textarea name="maintenance_detail" placeholder="Detail" id="elm1">{{@$maintenance->maintenance_detail}}</textarea>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="custom-file">
                                                <input name="maintenance_image" type="file" class="custom-file-input" id="customFile" accept="image/*" onchange="imgChange(this)" value="{{@$maintenance->maintenance_image}}" @empty($maintenance->maintenance_image) required @endempty >
                                                <label class="custom-file-label" for="customFile">@isset($maintenance->maintenance_image) {{$maintenance->maintenance_image}} @else Choose Picture @endisset</label>
                                            </div>
                                        <span class="required"> &nbsp; Suitable scale 800 x 533 pixels.</span>
                                        </div>
                                        <div class="form-group">
                                            <label for="nameImg">Name image <span class="required">*</span></label>
                                            <input type="text" class="form-control" value="{{pathinfo(@$maintenance->maintenance_image, PATHINFO_FILENAME)}}" id="nameImg" name="maintenance_image_name" required>
                                        </div>
                                        {{-- <div class="col-md-12"> --}}
                                            <div class="form-group">
                                                <img class="img-thumbnail imagePreview"@if(!isset($maintenance->maintenance_image)) style="display: none;" @endif alt="200x200" width="200" src="{{ URL::asset('upload/maintenance/'.@$maintenance->maintenance_image) }}" data-holder-rendered="true">
                                            </div>
                                        {{-- </div> --}}
                                    </div>
                                {{-- </div> --}}
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input id="metatitle" name="meta_title" type="text" placeholder="Detailed Title" class="form-control" value="{{@$maintenance->meta_title}}">
                                </div>
                                <div class="form-group">
                                    <label for="metakeywords">Meta Keywords</label>
                                    <input id="metakeywords" name="meta_keywords" type="text" placeholder="Detailed Keywords" class="form-control" value="{{@$maintenance->meta_keywords}}">
                                </div>
                            </div>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" id="meta_description" name="meta_description" placeholder="Meta Description" rows="5">{{@$maintenance->meta_description}}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {{-- <div class="row"@if (!isset($maintenance))
                style="
                pointer-events: none;
                opacity: 0.4;"
            @endif>
        ...
        <script>
            ...
            $(function(){
                uploader = new Dropzone(".dropzone",{
                    url: "{{url('admin/maintenance/uploadGallery')}}",
                    paramName : "file",
                    params: function params(files, xhr, chunk) { return { '_token' : "{{csrf_token()}}",'name':$('#maintenanceName').val() }; },
                ...
                // $.get("{{url('admin/maintenance/gallery')}}/{{@$maintenance->id}}", function(data) {
                ...
                uploader.on("removedfile", function(file) {
                    ...
                    for (var f = 0; f < fileList.length; f++) {
                        if (fileList[f].fileName == file.name) {
                            ...
                            $('#maintenance_gallerys').val(imageDataArray);
                            // get removed database file name
                            rmvFile = fileList[f].serverFileName;
                            // get request to remove the uploaded file from server
                            $.get( "{{url('admin/maintenance/deleteGallery')}}", { file: rmvFile } )
                            .done(function( data ) {
                                //console.log(data)
