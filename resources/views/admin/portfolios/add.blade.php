...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <form id="portfolioForm" action="{{$action}}" method="post" class="needs-validation outer-repeater" novalidate="" enctype="multipart/form-data">
    ...
    <div class="col-sm-12">
        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select required name="category_id" id="category" class="form-control">
                                                ...
                                                @foreach ($portfolio_category as $item)
                                                    <option @if (isset($portfolio->ref_category_id)) @if ($portfolio->ref_category_id == $item->id) selected @endif @endif value="{{$item->id}}">{{$item->portfolio_category_name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="custom-file">
                                                <input name="portfolio_image" type="file" class="custom-file-input" id="customFile" accept="image/*" onchange="imgChange(this)" value="{{@$portfolio->portfolio_image}}" @empty($portfolio->portfolio_image) required @endempty >
                                                <label class="custom-file-label" for="customFile">@isset($portfolio->portfolio_image) {{$portfolio->portfolio_image}} @else Choose Picture @endisset</label>
                                            </div>
                                        <span class="required"> &nbsp; Suitable scale 800 x 533 pixels.</span>
                                        </div>
                                        <div class="form-group">
                                            <label for="nameImg">Name image <span class="required">*</span></label>
                                            <input type="text" class="form-control" value="{{pathinfo(@$portfolio->portfolio_image, PATHINFO_FILENAME)}}" id="nameImg" name="portfolio_image_name" required>
                                        </div>
                                        {{-- <div class="col-md-12"> --}}
                                            <div class="form-group">
                                                <img class="img-thumbnail imagePreview"@if(!isset($portfolio->portfolio_image)) style="display: none;" @endif alt="200x200" width="200" src="{{ URL::asset('upload/portfolio/'.@$portfolio->portfolio_image) }}" data-holder-rendered="true">
                                            </div>
                                        {{-- </div> --}}
                                    </div>
                                {{-- </div> --}}
            ...
            {{-- <div class="row"@if (!isset($portfolio))
                style="
                pointer-events: none;
                opacity: 0.4;"
            @endif>
        ...
        <script>
            ...
            $(function(){
                uploader = new Dropzone(".dropzone",{
                    url: "{{url('admin/portfolio/uploadGallery')}}",
                    paramName : "file",
                    params: function params(files, xhr, chunk) { return { '_token' : "{{csrf_token()}}",'name':$('#portfolioName').val() }; },
                ...
                // $.get("{{url('admin/portfolio/gallery')}}/{{@$portfolio->id}}", function(data) {
                ...
                uploader.on("removedfile", function(file) {
                    ...
                    for (var f = 0; f < fileList.length; f++) {
                        if (fileList[f].fileName == file.name) {
                            ...
                            $('#portfolio_gallerys').val(imageDataArray);
                            // get removed database file name
                            rmvFile = fileList[f].serverFileName;
                            // get request to remove the uploaded file from server
                            $.get( "{{url('admin/portfolio/deleteGallery')}}", { file: rmvFile } )
                            .done(function( data ) {
                                //console.log(data)
