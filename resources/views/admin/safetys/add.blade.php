...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <form id="safetyForm" action="{{$action}}" method="post" class="needs-validation outer-repeater" novalidate="" enctype="multipart/form-data">
                                ...
                                <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select required name="category_id" id="category" class="form-control">
                                                ...
                                                @foreach ($safety_category as $item)
                                                    <option @if (isset($safety->ref_category_id)) @if ($safety->ref_category_id == $item->id) selected @endif @endif value="{{$item->id}}">{{$item->safety_category_name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    <div class="form-group">
                                        <label for="safetyName">Name <span class="required">*</span></label>
                                        <input type="text" name="safety_name" class="form-control" id="safetyName" placeholder="name" value="{{@$safety->safety_name}}" required>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="elm1">Detail</label>
                                        <textarea name="safety_detail" placeholder="Detail" id="elm1">{{@$safety->safety_detail}}</textarea>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="custom-file">
                                                <input name="safety_image" type="file" class="custom-file-input" id="customFile" accept="image/*" onchange="imgChange(this)" value="{{@$safety->safety_image}}" @empty($safety->safety_image) required @endempty >
                                                <label class="custom-file-label" for="customFile">@isset($safety->safety_image) {{$safety->safety_image}} @else Choose Picture @endisset</label>
                                            </div>
                                        <span class="required"> &nbsp; Suitable scale 800 x 533 pixels.</span>
                                        </div>
                                        <div class="form-group">
                                            <label for="nameImg">Name image <span class="required">*</span></label>
                                            <input type="text" class="form-control" value="{{pathinfo(@$safety->safety_image, PATHINFO_FILENAME)}}" id="nameImg" name="safety_image_name" required>
                                        </div>
                                        {{-- <div class="col-md-12"> --}}
                                            <div class="form-group">
                                                <img class="img-thumbnail imagePreview"@if(!isset($safety->safety_image)) style="display: none;" @endif alt="200x200" width="200" src="{{ URL::asset('upload/safety/'.@$safety->safety_image) }}" data-holder-rendered="true">
                                            </div>
                                        {{-- </div> --}}
                                    </div>
                                {{-- </div> --}}
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input id="metatitle" name="meta_title" type="text" placeholder="Detailed Title" class="form-control" value="{{@$safety->meta_title}}">
                                </div>
                                <div class="form-group">
                                    <label for="metakeywords">Meta Keywords</label>
                                    <input id="metakeywords" name="meta_keywords" type="text" placeholder="Detailed Keywords" class="form-control" value="{{@$safety->meta_keywords}}">
                                </div>
                            </div>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" id="meta_description" name="meta_description" placeholder="Meta Description" rows="5">{{@$safety->meta_description}}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <script>
            ...
            $(function(){
                uploader = new Dropzone(".dropzone",{
                    url: "{{url('admin/safety/uploadGallery')}}",
                    paramName : "file",
                    params: function params(files, xhr, chunk) { return { '_token' : "{{csrf_token()}}",'name':$('#safetyName').val() }; },
                ...
                // $.get("{{url('admin/safety/gallery')}}/{{@$safety->id}}", function(data) {
